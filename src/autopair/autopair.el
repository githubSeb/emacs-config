...
`autopair-wrap-action'. "
  (when (and autopair-wrap-action
    ...
    ;;   (undo)
    ;;   (backward-char)
    ;;   (setq azkae-autopair-c (line-number-at-pos (point)))
    ;;   (redo)
    ;;   )
    (save-excursion			;handle insert { while selectionning text
      (goto-char (region-beginning))
      (setq azkae-autopair-a (line-number-at-pos (point)))
      (goto-char (region-end))
      (forward-char)
      (setq azkae-autopair-b (line-number-at-pos (point)))
      )
    (cond ((and (equal (string (preceding-char)) "{")
		(not (equal azkae-autopair-a azkae-autopair-b))
		)
