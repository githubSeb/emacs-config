...
(defcustom ac-disable-faces '(font-lock-comment-face font-lock-doc-face)
...
(defcustom ac-modes
  '(emacs-lisp-mode
    ...
    perl-mode cperl-mode python-mode ruby-mode coffee-mode
    ecmascript-mode javascript-mode js-mode js2-mode php-mode css-mode
    makefile-mode sh-mode fortran-mode f90-mode ada-mode
    xml-mode sgml-mode cmake-mode)
...
(defvar ac-completing-map
  (let ((map (make-sparse-keymap)))
    (define-key map [tab] 'ac-complete) ; AZKAE - HERE WAS "ac-expand"
    (define-key map "\t" 'ac-complete) ; AZKAE - HERE WAS "ac-expand"
;    (define-key map "\r" 'ac-complete)
;    (define-key map "\r" 'azkae-do-nothing)
...
(defvar azkae-current-command nil)
...
(defun bad-ac-expand () ; OLD VERSION OF ac-expand, the new one is below
...
(defun ac-expand () ; OLD VERSION OF ac-expand, the new one is below
  "Try expand, and if expanded twice, select next candidate."
  (interactive)
  (unless (ac-expand-common)
    (let ((string (ac-selected-candidate)))
...
;	(if (and (equal "" prefix)
;		 (not (equal "ac-complete-clang-async" azkae-current-command))
;		 (not (equal "ac-clang-async-autocomplete-autotrigger" azkae-current-command)))
...
(defun* ac-start (&key
  ...
  (if (not auto-complete-mode)
	...
	(if (and (equal "" prefix)
		 (equal "c-electric-backspace" azkae-current-command)
		 )
	    (ac-abort))

;	(message "AZ:%s" ac-clang-current-candidate)
	
; AZKAE WIP ON DELETING AUTO COMPLETE ON BACKSPACE
	(if (and (equal "" prefix)
		     (not (equal "ac-complete-clang-async" azkae-current-command))
		     (not (equal "ac-clang-async-autocomplete-autotrigger" azkae-current-command))
		     (not (equal "ac-complete" azkae-current-command)))
	    (unless ac-cursor-color
	    (setq ac-cursor-color (frame-parameter (selected-frame) 'cursor-color)))
...
;  (if (not (equal 'c-electric-backspace command))
;      (if (not (equal 'c-electric-star command))
;	  (if (not (equal 'c-electric-paren command))
	      (and (symbolp command)   (setq azkae-current-command (symbol-name command))
...
;; Dat dirty code! -- shows my love for lisp :D
...
(defun ac-makefile-rules-candidate()
  ;; (print "azkae-makefile-line")
  ;; (setq azkae-makefile-line (split-string
  ;; 			     (buffer-string) "\n" t))
  ;; (loop for line in azkae-makefile-line
  ;; 	when (not (equal line nil))
  ...
  (save-excursion
    ...
    (while azkae-makefile-parse-line
      ...
      ;; (if (not (equal (thing-at-point 'word) nil))
      	  ;; (setq azkae-makefile-list-rules (append
      	  ;; 	azkae-makefile-list-rules (thing-at-point 'word)))
      	;; )
      (setq azkae-makefile-parse-line (= 0 (forward-line 1)))
      )
    ;; (print azkae-makefile-list-rules)
    ;; (print ac-prefix)
    )
