...
static int completion_printCompletionHeadTerm(
    CXCompletionString completion_string, FILE *fp, const char *prefix, size_t len_prefix)
{
    int i_chunk  = 0;
    ...
    for ( ; i_chunk < n_chunks; i_chunk++)
    {
	    ...
	    result = clang_getCString(ac_string);
	    if (!strncmp(prefix, result, len_prefix))
	    {
		fprintf(fp, "COMPLETION: %s", result);
    ...
    CXCompletionString completion_string, FILE *fp, const char *prefix, size_t len_prefix)
{
    ...
    if (completion_printCompletionHeadTerm(completion_string, fp, prefix, len_prefix) > 1)
    {
					   ...
					   const char *prefix, size_t len_prefix)
{
    ...
    for ( ; i < res->NumResults; i++) {
        completion_printCompletionLine(res->Results[i].CompletionString, fp, prefix, len_prefix);
    }
}
